...
class WasabiMotionReference:
    ...
    The class deliberately owns only motion data. IsaacLab event terms call
    :meth:`reset` and :meth:`advance`, while observation terms read the
    ...
    def _gather(self, attr: str) -> torch.Tensor:
        clips = [getattr(clip, attr) for clip in self._clips]
        result = torch.empty(
            (self.num_envs, *clips[0].shape[1:]), device=self.device, dtype=clips[0].dtype
        )
    ...
    def joint_pos_relative(self, default_joint_pos: torch.Tensor) -> torch.Tensor:
        return self.joint_pos - default_joint_pos
